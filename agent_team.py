...
class CreativeTeam:
    ...
    ECD_CD_BRIEF_SYSTEM = (
        ...
        "まず『戦略方向性マップ』を作り、そこから1案を選択してCDブリーフに落とす。\n\n"
        "【Step 1: 戦略方向性マップ（必須）】\n"
        "可能性のある方向性を3案並べる。各案に:\n"
        "・タイトル（キャッチーな一言）\n"
        "・一行説明（何を問いにするか）\n"
        "・使うインサイト仮説（Researcherの何番か）\n"
        "・使う戦略テリトリー（Strategic Plannerの何案か）\n"
        "・面白い理由と難しい理由\n\n"
        "【Step 2: 選択と理由の明示（必須）】\n"
        "3案の中から最も強い1案を選び、『なぜこれか』と『なぜ他を捨てたか』を明記する。\n\n"
        "【Step 3: CDブリーフ（選択した方向性のみ）】\n"
        "・書き換えられた問い（確定版）\n"
        "・禁じ手リスト\n"
        "・採用するインサイト（「実は〜」の形で）\n"
        "・異業種移植アイデア\n"
