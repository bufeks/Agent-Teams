    ...
    def __init__(self, client: anthropic.Anthropic):
        super().__init__(
            ...
            system_prompt=(
                ...
                "4. 意外な事実：このブリーフに関係する、見落とされている真実\n\n"
                "【重要：1つに絞らない】\n"
...
class CreativeTeam:
    ...
    ECD_CD_BRIEF_SYSTEM = (
        ...
        "【戦略方向性マップのルール】\n"
        "・方向性は最低3案出す\n"
        "・うち1案は『ブリーフが期待している方向とは別の問い立て』から出発すること（カテゴリーの土俵を変える案、ターゲットの切り口を変える案など）\n"
        "・似た方向性の並列は不可。各案は構造的に異なること（異なるインサイト深度・異なる文化的緊張・異なるターゲット状態など）\n\n"
        "【各方向性に付けるセット】\n"
        "・タイトル：方向性を表す一言\n"
        "・インサイト：「実は〜」の形で（Researcherの仮説から選ぶ）\n"
        "・メリット：なぜこれが面白いか、刺さりうるか、カンヌ水準になりうるか\n"
        "・デメリット・ハードル：難しさ、リスク、クライアント合意が必要な点\n"
        "・この方向を選ぶと捨てることになるもの：戦略的トレードオフ（どの訴求・感情・ターゲットを諦めるか）\n"
        "・例えばの表現：この方向性に行ったときに生まれうるコピーや表現のラフスケッチ\n\n"
        "【方向性マップを出し終えたら必ず以下を出力する】\n"
        "---\n"
        "## どの方向性で進めますか？\n\n"
        "A案・B案・C案のいずれかを選ぶか、「AとBを組み合わせたい」「別の方向を試したい」など、\n"
        "方向性を教えてください。選択後、CDブリーフから再開します。\n"
        "---\n\n"
        "【CDブリーフ（方向性が決まってから出力）】\n"
        "・禁じ手リスト（どの方向に行っても守るべきこと）\n"
        "・インサイト候補一覧（方向性ごとに対応するインサイト）\n"
        "・異業種移植アイデア\n"
        "・ECDとしての期待水準"
    )
