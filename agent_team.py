    ...
    def __init__(self, client: anthropic.Anthropic):
        super().__init__(
            ...
            system_prompt=(
                ...
                "Step 1 — 機能語の封印：\n"
                "「涼しい」「軽い」「通気性」「接触冷感」「速乾」など、機能・スペック・比較を表す言葉を一切使わずに、"
                "この商品・このコレクションを着ている人の身体感覚を描写せよ。"
                "使える言葉は自然・天気・素材・動作・感触の言葉だけ。\n"
                "Step 2 — 自然言語で掴む：\n"
                "その感覚に最も近い自然現象・季節・素材・動作を一語で言え（例：風、霧、影、水、そよぎ）。"
                "この一語がコレクション全体に成立するか確認する。一商品のFACTに縛られていないか確認する。\n"
                "Step 3 — 動詞と方向を添える：\n"
                "その一語を軸に、身体の動き（着る・まとう・連れる）と方向（〜へ・〜のなかへ・〜とともに）を組み合わせる。\n"
